...
void HalSimu::updateEncoder(){
    ...
    encoder_1_ = encoder_1_ + 
        (saved_speed_1_- 127) *
        ratio_speed_time_ *
        std::chrono::duration_cast<std::chrono::milliseconds>
        (time_update - timestamp_since_last_encoder_1_update_).count();
    encoder_2_ = encoder_2_ + 
        (saved_speed_2_- 127) *
        ratio_speed_time_ *
        std::chrono::duration_cast<std::chrono::milliseconds>
        (time_update - timestamp_since_last_encoder_2_update_).count();
