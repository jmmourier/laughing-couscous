...
CommandData MessageParser::analyseBuffer() {
    ...
    if (position_of_end_char == std::string::npos) {
        // std::cout << "[MessageParser] no end char found during analyse" <<
        // std::endl;
    } else {
        // std::cout << "[MessageParser] end char found during analyse" <<
        // std::endl;
        ...
        if (position_of_end_char == 0) {
            ...
            position_of_first_split_char == std::string::npos ||
            position_of_first_split_char > position_of_end_char) {
            // std::cout << "[MessageParser] no split char found during analyse" <<
            // std::endl;
            ...
            if (position_of_end_char > position_of_first_split_char + 1) {
                returned_command_data.arguments_.emplace_back(buffer_.substr(
                    position_of_first_split_char + 1,
                    position_of_end_char - position_of_first_split_char - 1));
            }
...
bool MessageParser::isCharAllowed(char testedChar) {
    ...
    return (
        allowedLetters.find(testedChar, 0) != std::string::npos ||
        allowedNumbers.find(testedChar, 0) != std::string::npos ||
        allowedPonctuation.find(testedChar, 0) != std::string::npos || testedChar == endChar_ ||
