...
TEST(NaviStateMachineTest, start_align_to_target_1) {
    ...
    EXPECT_EQ(
        enumNaviStateMachine::ST2_ALIGN_TO_TARGET,
        state1_start_align_to_target(robot_pos, target_pos, dummy_lambda));
}
...
TEST(NaviStateMachineTest, align_to_target_2) {
    ...
    EXPECT_EQ(
        enumNaviStateMachine::ST2_ALIGN_TO_TARGET,
        state2_align_to_target(robot_pos, target_pos, dummy_lambda));
    // turning done
    robot_pos.orientation = M_PI / 4;
    EXPECT_EQ(
        enumNaviStateMachine::ST3_WAIT_FOR_MOVMENT,
        state2_align_to_target(robot_pos, target_pos, dummy_lambda));
...
TEST(NaviStateMachineTest, driving_to_target_5) {
    ...
    // target not reached
    EXPECT_EQ(
        enumNaviStateMachine::ST5_DRIVING_TO_TARGET,
    ...
    EXPECT_EQ(
        enumNaviStateMachine::ST6_WAIT_FOR_ROTATION,
        state5_driving_to_target(robot_pos, target_pos, dummy_lambda));
}
...
TEST(NaviStateMachineTest, rotation_to_target_orientation_8) {
    ...
    // turning in progress
    EXPECT_EQ(
        enumNaviStateMachine::ST8_ROTATION_TO_TARGET_ORIENTATION,
        state8_rotation_to_target_orientation(robot_pos, target_pos));
    // turning done
    robot_pos.orientation = 1.0;
    EXPECT_EQ(
        enumNaviStateMachine::ST9_DONE,
        state8_rotation_to_target_orientation(robot_pos, target_pos));
}