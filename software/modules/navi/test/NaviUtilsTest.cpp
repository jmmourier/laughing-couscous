...
TEST(NaviUtilsTest, compute_angle_with_target) {
    ...
    target_pos_x = 100.0;
    target_pos_y = 100.0;
    double angle = get_angle_to_target(
        robot_pos_x,
        robot_pos_y,
        robot_orientation,
        target_pos_x,
...
TEST(NaviUtilsTest, utils_rotation_direction) {
    ...
    EXPECT_EQ(
        ...
        get_rotation_direction(M_PI / 4.f, 3 * (M_PI / 4.f)));

    // case: robot_orientation > target_orientation. diff > PI
    EXPECT_EQ(
        rotation_direction::Clockwise,
        get_rotation_direction(M_PI + (M_PI / 3.f), M_PI / 2.f));
    // case: robot_orientation < target_orientation. diff > PI
    EXPECT_EQ(
        rotation_direction::AntiClockwise,
        get_rotation_direction(M_PI / 3.f, M_PI + 3 * (M_PI / 4.f)));
